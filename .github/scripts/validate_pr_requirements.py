...
def validate_pr_requirements(config_path, pr_number, target_branch, team_name, github_token):
    ...
    try:
        for team_name, info in team_info.items():
            if info['exists']:
                try:
                    # Get the team object and add it as a reviewer
                    team = info['team_object']
                    pull_request.add_to_reviewers(team_ids=[team.id])
                    print(f"Added team {team_name} as reviewer")
                except GithubException as e:
                    print(f"Error adding team {team_name} as reviewer: {e}")
    except Exception as e:
    ...
    valid_approvals = sum(1 for member in all_team_members 
