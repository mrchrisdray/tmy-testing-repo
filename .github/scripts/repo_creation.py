...
class RepositoryCreationHandler:
    ...
    def generate_repository_config(self, input_data):
        ...
        if input_data.get('collaborators'):
            config['collaborators'] = input_data['collaborators'].split('\n')
        
        # Add optional teams
        if input_data.get('teams'):
            config['teams'] = input_data['teams'].split('\n')
...
def handle_repository_creation_issue(issue, github_token, org_name):
    ...
    # Parse issue body using more robust method
    input_data = {}
    ...
    # Extract from GitHub issue form inputs
    form_inputs = {}
    for line in issue.body.split('\n'):
        # Check for GitHub issue form input markers
        if line.startswith('### '):
            current_section = line[4:].lower().replace(' ', '_')
        elif ':' in line and current_section:
            key, value = line.split(':', 1)
            form_inputs[current_section] = value.strip()
    
    # Merge or prioritize form inputs
    input_data.update(form_inputs)
    
    # Validate inputs
    validation_results = {
        'name': handler.validate_repository_name(input_data.get('repo-name', '')),
    ...
    if all(result[0] for result in validation_results.values()):
        ...
        config = handler.generate_repository_config({
...
def main():
    ...
    try:
        # Fetch the repository containing the issue
        repo = g.get_repo(f"{org_name}/{org_name}.github")
